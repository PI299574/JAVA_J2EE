...
Minimum Platforms needed on railway station = Maximum platforms 
                                           ...
                                           = 3  
*/
public class Min_No_Of_Plateform_Rqrd {
	...
	private static int findMinNoOfPlateformRequired1(int[] arr, int[] dep) {
		// TODO Auto-generated method stub
		int pltfrm_rqrd=0;
	...
	private static int findMinNoOfPlateformRequired(int[] arr, int[] dep) {
		...
		int arr_Inex=0;int dept_Index=0;int min_pltfm_reqd=0; int max_value=0;
		while(arr_Inex<arr.length && dept_Index<dep.length){
			if(arr[arr_Inex]<dep[dept_Index]){
				arr_Inex++;
				min_pltfm_reqd++;
			}else{
				dept_Index++;
			...
			if(max_value<min_pltfm_reqd){
				max_value=min_pltfm_reqd;
			}
		}
